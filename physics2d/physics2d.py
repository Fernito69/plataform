...
class Physics2D:
    ...
    def render_scenario(self) -> None:
        ...
        for y in range(0, self.display.curr_y_resolution - 1, 2):
            backwards_y = self.display.curr_y_resolution - 1 - y
            new_y = int(y / 2)

            if len(new_screen_matrix) <= new_y:
                new_screen_matrix.append([])
            for x in range(self.display.curr_x_resolution - 1):
                raise NotImplementedError(
                    f"y={len(self.screen_buffer)} x={len(self.screen_buffer[0])} | backwards_y={backwards_y}"
                )
                ...
                if not upper_pixel_info and not lower_pixel_info:
                    new_screen_matrix[new_y][x] = EMPTY_SPACE
                    continue
